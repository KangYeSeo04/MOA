...
export default function BurgerMenuScreen() {
  ...
  const setTotal = useCartStore((s) => (s as any).setTotal); // ⚠️ stores/cart.ts에 setTotal 있어야 함
  ...
  const [restaurantName, setRestaurantName] = useState<string>(name ?? FALLBACK_RESTAURANT_NAME);
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      try {
        ...
        if (typeof st?.pendingPrice === "number" && typeof setTotal === "function") {
  ...
  const patchPendingPrice = async (delta: number) => {
    ...
    if (st && typeof st.pendingPrice === "number" && typeof setTotal === "function") {
      setTotal(restaurantId, st.pendingPrice);
    }
